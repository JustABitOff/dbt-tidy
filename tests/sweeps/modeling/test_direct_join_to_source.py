...
class TestDirectJoinToSourcePass:
    ...
    def _run_cli(
        ...
        result = runner.invoke(cli, ["sweep", "--include", "direct_join_to_source"])

        assert result.exit_code == 0


class TestDirectJoinToSourceFail:
    def test_manifestv10(
        self, manifestv10_direct_join_to_source_fixture, tidy_config_fixture
    ):
        self._run_cli(
            mocked_manifest=manifestv10_direct_join_to_source_fixture,
            tidy_config_fixture=tidy_config_fixture,
        )

    def test_manifestv11(
        self, manifestv11_direct_join_to_source_fixture, tidy_config_fixture
    ):
        self._run_cli(
            mocked_manifest=manifestv11_direct_join_to_source_fixture,
            tidy_config_fixture=tidy_config_fixture,
        )

    def test_manifestv12(
        self, manifestv12_direct_join_to_source_fixture, tidy_config_fixture
    ):
        self._run_cli(
            mocked_manifest=manifestv12_direct_join_to_source_fixture,
            tidy_config_fixture=tidy_config_fixture,
        )
